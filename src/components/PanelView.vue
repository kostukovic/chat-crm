<script setup lang="ts">
  ...
  const title = computed(() => {
    if (sel.module==='telegram' && sel.item==='nataschaa') return 'Thema 1 – Design‑Review & Prototyping'
    if (sel.module==='telegram' && sel.item==='kostja')    return 'Thema 2 – Messdaten & Auswertung'
    if (sel.module==='email'    && sel.item==='nina')      return 'Rechnung A – Bestellübersicht & Rückfragen'
    if (sel.module==='email'    && sel.item==='goscha')    return 'Rechnung B – Adresskorrektur & Prozess'
    if (sel.module==='calendar' && sel.item==='wichtig')   return 'Agenda – Projekt‑Sync 14:30'
    return 'Details'
  })
  ...
  <template>
    <section class="panel" v-show="ui.bp===2 && ui.showPanel || ui.bp===0 && ui.mobileMode==='panel'">
      <header style="display:flex;align-items:center;justify-content:space-between;padding:.6rem .8rem;border-bottom:1px solid #1e2733">
        <button v-if="ui.bp<=1" class="back-btn" @click="goBack" aria-label="Zurück">←</button>
        <strong>📎 {{ title }}</strong>
        <div style="display:flex;gap:.4rem">
          <button v-if="ui.bp===2" class="btn" @click="ui.setFullscreen(ui.fullscreen==='panel'?null:'panel')">{{ ui.fullscreen==='panel' ? 'Vollbild verlassen' : 'Vollbild' }}</button>
          <button v-if="ui.bp===0" class="btn" @click="ui.setMobileMode('main')">Zurück</button>
        </div>
      </header>
  
      <div style="padding:1rem; line-height:1.55; max-width: 72ch;">
        <p v-for="(p,i) in paragraphs" :key="i" style="margin:.75rem 0">{{ p }}</p>
      </div>
    </section>
  </template>

  <style scoped>
    .pv-header{ display:flex; align-items:center; justify-content:space-between; padding:.6rem .8rem; border-bottom:1px solid #1e2733 }
    .back-btn{ appearance:none; border:1px solid #243041; background:#17202b; color:#e6edf3; padding:.35rem .55rem; border-radius:.6rem; margin-right:.5rem; cursor:pointer }
    @media (min-width: 1367px){ .back-btn{ display:none } }
  </style>