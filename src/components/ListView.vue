<!-- src/components/SidebarList.vue -->
...
<script setup lang="ts">
  ...
  <template>
    ...
    <section class="list" v-show="ui.bp>=1 || ui.bp===0 && ui.showList">
      <header class="lv-header">
        <strong>{{ ui.active }}</strong>
        <button v-if="ui.bp>=1" class="toggle-list-btn" @click="collapse" aria-label="Liste einklappen" title="Liste einklappen">&lt;</button>
      </header>
  ...
  <style scoped>
    .lv-header{ display:flex; justify-content:space-between; align-items:center; padding:.6rem .8rem; border-bottom:1px solid #1e2733 }
    .toggle-list-btn{
      appearance:none; border:1px solid #243041; background:#17202b; color:#e6edf3;
      padding:.35rem .55rem; border-radius:.6rem; cursor:pointer;
    }
    @media (max-width: 767px){ .toggle-list-btn{ display:none } }
  </style>